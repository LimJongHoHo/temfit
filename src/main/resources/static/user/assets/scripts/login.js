...
$loginForm.onsubmit = (e) => {
    ...
    xhr.onreadystatechange = () => {
        ...
        if (xhr.status < 200 || xhr.status >= 300) {
            dialog.showSimpleOk('로그인', '요청을 처리하는 도중 오류가 발생하였습니다. 잠시 후 다시 시도해 주세요.');
            return;
        }
        ...
        switch (response.result) {
            ...
            case 'success':
                ...
                location.href = new URL(location.href).searchParams.get('forward') ?? '/main';
                break;
            default:
                dialog.showSimpleOk('로그인', '이메일 혹은 비밀번호가 올바르지 않습니다. 다시 확인해 주세요.')
        }
    };
